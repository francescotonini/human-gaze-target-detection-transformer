...
def crop(image, target, p=0.5):
    if np.random.random_sample() > p:
        return image, target

    # Do not crop if there are no boxes
    if target["boxes"] is None or target["gaze_points"] is None:
        return image, target
...
def resize(image, target, size, max_size=None):
    ...
    if target["boxes"] is not None and target["gaze_points"] is not None:
        for box_name in ["boxes"]:
            if box_name in target:
                boxes = target[box_name]
                boxes = boxes * torch.FloatTensor(
...
def hflip(image, target):
    ...
    # Do not flip boxes and point if there are no boxes or points
    if target["boxes"] is None or target["gaze_points"] is None:
...
class RandomCrop(object):
    ...
    def __call__(self, img, target=None):
        return crop(img, target, p=self.p)
...
class Compose(object):
    ...
    def __call__(self, image, target):
        if target["path"] == "/datasets/videocoatt/images/train/124/00242_124.jpg":
            breakpoint()
