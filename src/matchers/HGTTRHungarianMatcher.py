...
class HGTTRHungarianMatcher(nn.Module):
    def __init__(
        self,
        # alphas
    ...
    def forward(self, outputs, targets, eval_mode=False):
        ...
        out_watch_outside = (
            outputs["pred_gaze_watch_outside"].flatten(0, 1).softmax(-1)
        )  # [batch_size * num_queries, num_classes]
        ...
        cost_watch_outside = -out_watch_outside[:, tgt_watch_outside.argmax(-1)]
        cost_bbox = self.loss_bbox_coeff * torch.cdist(out_bbox, tgt_bbox, p=1)
        cost_gaze_heatmap = torch.cdist(out_gaze_heatmap, tgt_heatmap, p=2)

        # Compute the giou cost between target
